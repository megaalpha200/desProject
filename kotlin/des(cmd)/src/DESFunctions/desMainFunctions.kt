...
fun desPrep(cipherMode: CipherMode, inputText: String, key: String, swapLastRound: Boolean) : String {
    ...
    println("${if (cipherMode == CipherMode.ENCRYPT) "PlainText" else if (cipherMode == CipherMode.DECRYPT) "CipherText" else ""} Binary: ${inputTextBinChunked.joinToString(" | ") { it.chunked(4).joinToString(" ") }}")
...
private fun desBinEncryptDecrypt(cipherMode: CipherMode, inputTextBin: String, roundKeys: List<String>, swapLastRound: Boolean) : String {
    ...
    for (round in 0 until 16) {
        ...
        println("${if (cipherMode == CipherMode.ENCRYPT) "CipherText" else if (cipherMode == CipherMode.DECRYPT) "PlainText" else ""} after Round ${(round + 1).toString().padStart(2, '0')}: ${convertBinToHex(tempRoundInputText).chunked(8).joinToString(" ")}")
        println(System.lineSeparator())
    }
