...
/*----------------------------------------------------------------------------*/
    ...
    enum class CipherMode (value: Int) {
        ENCRYPT(0), DECRYPT(1);
    ...
    var debugOutput = StringBuilder("")

    fun desPrep(cipherMode: CipherMode, inputText: String, key: String, swapLastRound: Boolean) : String {
        val finalResult = StringBuilder("")
        val inputTextBin = inputText
        val inputTextBinChunked = inputTextBin.chunked(64) { it.padEnd(64, '0') }
        debugOutput.append("${if (cipherMode == CipherMode.ENCRYPT) "PlainText" else if (cipherMode == CipherMode.DECRYPT) "CipherText" else ""} Binary: ${inputTextBinChunked.joinToString(" | ") { it.chunked(4).joinToString(" ") }}${System.lineSeparator()}")

        val keyBin = key.padEnd(64, '0')
        //debugOutput.append("Key Binary: ${keyBin.chunked(8).joinToString(" ")}")

        val roundKeys = generateRoundKeys(keyBin, cipherMode)
        debugOutput.append(System.lineSeparator())

        inputTextBinChunked.forEachIndexed { index, block ->
            debugOutput.append("For block ${index + 1}...${System.lineSeparator()}")

            finalResult.append(desBinEncryptDecrypt(cipherMode, block.toString(), roundKeys, swapLastRound))
        }

        return finalResult.toString()
    }

    private fun desBinEncryptDecrypt(cipherMode: CipherMode, inputTextBin: String, roundKeys: List<String>, swapLastRound: Boolean) : String {
        ...
        for (round in 0 until 16) {
            ...
            tempRoundInputText = if ((cipherMode == CipherMode.ENCRYPT && round + 1 == 16) || (cipherMode == CipherMode.DECRYPT && round + 1 == 1))
                roundFunction(cipherMode, tempRoundInputText, roundKeys[round], swapLastRound)
            else
                roundFunction(cipherMode, tempRoundInputText, roundKeys[round])

            debugOutput.append("${if (cipherMode == CipherMode.ENCRYPT) "CipherText" else if (cipherMode == CipherMode.DECRYPT) "PlainText" else ""} after Round ${(round + 1).toString().padStart(2, '0')}: ${convertBinToHex(tempRoundInputText).chunked(8).joinToString(" ")}${System.lineSeparator()}")
            debugOutput.append(System.lineSeparator())
        }
...
/*----------------------------------Key Functions-------------------------------------*/
    ...
    private fun generateRoundKeys(key: String, cipherMode: CipherMode) : List<String> {
        ...
        for(round in 0..15) {
            debugOutput.append("Round ${(round+1).toString().padStart(2,'0')} Key: ${roundKeys[round].chunked(6).joinToString(" ")}${System.lineSeparator()}")
        }
...
/*--------------------------------Round Functions-------------------------------------*/
    ...
    private fun roundFunction(cipherMode: CipherMode, input: String, key: String, swap: Boolean = true) : String {
        ...
        debugOutput.append("Pre-Round Input: ${convertBinToHex(leftInput)} ${convertBinToHex(rightInput)}${System.lineSeparator()}")
    ...
    private fun innerRoundFunction(input: String, key: String) : String {
        ...
        debugOutput.append("Expansion Result: ${convertBinToHex(expandedInput)}${System.lineSeparator()}")
        val inputWithAppliedKey = xorBinaryBlocks(expandedInput, key)
        debugOutput.append("Applied Key Result: ${convertBinToHex(inputWithAppliedKey)}${System.lineSeparator()}")
