	...
	do
	{
		try
		{
			...
			if (mode == Mode::ENCRYPT)
				encrypt(inputTextAndKeyPair.first, inputTextAndKeyPair.second, swapLastRound);
			else if (mode == Mode::DECRYPT)
				decrypt(inputTextAndKeyPair.first, inputTextAndKeyPair.second, swapLastRound);
...
void encryptDecryptStringInputTextPrep(Mode mode, pair<string, string> &inputTextAndKeyPair)
{
	...
	string inputTextBin = convertStringToBin(inputText);
	...
	string initialKeyBin = convertStringToBin(initialKey);

	cout << endl;
	outputStream << endl;

	inputTextAndKeyPair = pair<string, string>(inputTextBin, initialKeyBin);
...
void encryptDecryptHexInputTextPrep(Mode mode, pair<string, string> &inputTextAndKeyPair)
{
	...
	string inputTextBin = convertHexToBin(inputText);
	...
	cout << "Binary: " << joinToString(chunkedInputTextBin, " ") << endl;
	...
	outputStream << "Initial Key: " << initialKey << endl;
	outputStream << "Initial Key Hex and Binary Representations:" << endl;
	string initialKeyBin = convertHexToBin(initialKey);
...
{
	string cipherTextHex = convertBinToHex(desPrep(Mode::ENCRYPT, plainTextBin, initialKeyBin, swapLastRound));
...
string desPrep(Mode mode, string inputText, string key, bool swapLastRound)
{
	string finalResult = "";
	...
	outputStream << ((mode == Mode::ENCRYPT) ? "PlainText" : "CipherText") << " Binary: " << joinToString(inputTextBinChunked, " | ") << endl;
...
string desBinEncryptDecrypt(Mode mode, string inputTextBin, vector<string> &roundKeys, bool swapLastRound)
{
	...
	for (int round = 0; round < 16; round++)
	{
		...
		outputStream << ((mode == Mode::ENCRYPT) ? "CipherText" : "PlainText") << " after Round " << padLeftWithZeros(to_string(round + 1), 2) << ": ";
...
void generateRoundKeys(string key, Mode mode, vector<string> &roundKeys)
{
