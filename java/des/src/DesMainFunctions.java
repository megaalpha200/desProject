...
public class DesMainFunctions {
    ...
    public static String desPrep(Mode mode, String inputText, String key, boolean swapLastRound) {
        ...
        System.out.println((mode == Mode.ENCRYPT) ? "PlainText" : "CipherText" + " Binary: " + String.join(" | ", inputTextBinChunked));

        final String keyBin = StringManipulations.padRightWithZeros(key, 64);

        final List<String> roundKeys = generateRoundKeys(keyBin, mode);
        System.out.println(System.lineSeparator());

        int blockNum = 1;
        for (String block : inputTextBinChunked) {
            System.out.println("For block " + blockNum + "...");

            finalResult.append(desBinEncryptDecrypt(mode, block, roundKeys, swapLastRound));
        }

        return finalResult.toString();
    }

    private static String desBinEncryptDecrypt(Mode mode, String inputTextBin, List<String> roundKeys, boolean swapLastRound) {
    ...
    private static List<String> generateRoundKeys(String key, Mode mode) {
        ...
        for (int round = 0; round < 16; round++) {
            System.out.println("Round " + StringManipulations.padLeftWithZeros(String.valueOf(round+1), 2) + " Key: " + String.join(" ", StringManipulations.chunkString(roundKeys.get(round), 6)));
        }
    ...
    private static String roundFunction(String input, String key, boolean swap) {
        ...
        System.out.println("Pre-Round Input: " + TextConversions.convertBinToHex(leftInput) + " " + TextConversions.convertBinToHex(rightInput));
    ...
    private static String innerRoundFunction(String input, String key) {
        ...
        System.out.println("Expansion Result: " + TextConversions.convertBinToHex(expandedInput));
        final String inputWithAppliedKey = xorBinaryBlocks(expandedInput, key);
        System.out.println("Applied Key Result: " + TextConversions.convertBinToHex(inputWithAppliedKey));
        ...
        for (String chunk: chunkedInputWithAppliedKey) {
            ...
            System.out.println("S-Box " + (index + 1) + " input: " + chunk);
            System.out.println("S-Box " + (index + 1) + " output: " + sBoxResult);
